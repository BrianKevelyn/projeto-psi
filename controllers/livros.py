...
def lista_livros():
    ...
    emprestimos_usuario = Emprestimo.query.filter_by(
...
def excluir_livro(livro_id):
    ...
    emprestimos_existentes = Emprestimo.query.filter_by(
        emp_liv_id=livro.liv_id, status="Em andamento"
    ).first()
    if emprestimos_existentes:
        flash(
            f"Não é possível excluir '{livro.liv_titulo}' porque existem empréstimos ativos.",
            "error",
        )
        return redirect(url_for("livros.lista_livros"))
...
def emprestimo_livro():
    ...
    return render_template(
        "emprestimo_livro.html", livros=livros_disponiveis, emprestimos=emprestimos
    )
...
def emprestimo_por_livro(liv_id):
    ...
    return render_template(
        "emprestimo_livro.html", livro=livro, emprestimos=emprestimos
    )
